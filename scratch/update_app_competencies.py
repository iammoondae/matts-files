...
# Define the new Grade 3 Three-Term competencies structure
new_g3_competencies = """  g3: {
    ...
    english: {
      ...
      "Term 3": [
        ...
        "EN3-T3-W6-10: Form degrees of comparison of adjectives, use prepositions, identify cause-and-effect, and write simple letters or journal entries"
      ]
    },
    filipino: {
      "Term 1": [
...
# Locate and replace the g3 competencies block using regex
# We look for the start of `g3: {` and replace up to its end `}` before `  }\n};`
pattern = r'g3:\s*\{.*?math:\s*\{.*?\}\s*\}'
content_new, count = re.subn(pattern, new_g3_competencies, content, flags=re.DOTALL)
if count > 0:
    print("Substituted MATATAG_COMPETENCIES g3 successfully!")
else:
    print("[ERROR] Failed to substitute MATATAG_COMPETENCIES g3!")

# Locate and replace the const quarters declaration in showCompetenciesModal
...
if quarters_old in content_new:
    ...
    print("Substituted quarters display list successfully!")
else:
    print("[ERROR] Failed to find quarters list inside app.js!")

with open(js_path, "w", encoding="utf-8") as f:
    f.write(content_new)

print("Finished app.js updates.")
